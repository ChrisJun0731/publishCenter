...
public class Util {
	 ...
	 * 合并多个byte[]数组
	 *
	 * @param list 存放多个byte[]数组的list
	 * @return 合并后的byte[]数组
	 */
//	public byte[] mergeByteArrays(List list) {
//		byte[] array = {};
//		for (int i = 0; i < list.size(); i++) {
//			byte[] ele = (byte[]) list.get(i);
//			int start = array.length;
//			array = Arrays.copyOf(array, array.length + ele.length);
//			System.arraycopy(ele, 0, array, start, ele.length);
//		}
//		return array;
//	}
	 ...
	 * @return
	 */
	public byte[] combineFrame(int block_size, int block_num, byte[] block) {
		...
		byte[] block_size_byte = convertInt2Byte(block_size);
		byte[] block_num_byte = convertInt2Byte(block_num);
		byte[] crc = new byte[2];
	 ...
	 * @return
	 */
	public byte[] escape(String config_json) {
		byte[] input = config_json.getBytes();
		int extra = 0;
